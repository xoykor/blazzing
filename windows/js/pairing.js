...
/*
 * Secure phone pairing for the Windows desktop port.
 *
...
(function () {
    ...
    function request(method, url, body, timeout) {
        ...
        if (!native || !native.netRequest) {
            return Promise.reject(new Error("A ponte de rede do Windows não está disponível."));
        }
        return native.netRequest(method, url, body, {
    ...
    function start(onSubmit, onState, baseUrl) {
        ...
        if (!window.crypto || !window.crypto.getRandomValues ||
            ...
            return Promise.reject(
                new Error("Web Crypto não está disponível neste Windows.")
            );
        }
