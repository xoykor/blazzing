...
int main(void) {
    ...
    fputs("#EXTM3U\n"
          ...
          "#EXTINF:-1 tvg-logo=\"https://img/dragon.jpg\" group-title=\"Series | Crunchyroll\",Dragon Ball Super S01 E01\n"
          "https://stream/dbs-s01e01.ts\n"
          "#EXTINF:-1 tvg-logo=\"https://img/dragon.jpg\" group-title=\"Series | Crunchyroll\",Dragon Ball Super S01 E02\n"
    ...
    TEST_CHECK(vip_m3u_parse_episode_label("Dragon Ball Super S01 E12", series_name, sizeof(series_name), &season, &episode));
    TEST_CHECK(strcmp(series_name, "Dragon Ball Super") == 0);
    TEST_CHECK(season == 1 && episode == 12);
    TEST_CHECK(vip_m3u_parse_episode_label("The Show - 2x03", series_name, sizeof(series_name), &season, &episode));
    ...
    vip_category_list_init(&live_cats); vip_channel_list_init(&live_channels);
    vip_category_list_init(&vod_cats); vip_channel_list_init(&vod_channels);
    vip_category_list_init(&series_cats); vip_channel_list_init(&series_channels);
    TEST_STATUS(vip_m3u_split_catalog(&cats, &channels,
                                      &live_cats, &live_channels,
                                      &vod_cats, &vod_channels,
                                      &series_cats, &series_channels,
                                      &error), VIP_OK, &error);
    ...
    vip_category_list_clear(&live_cats); vip_channel_list_clear(&live_channels);
    vip_category_list_clear(&vod_cats); vip_channel_list_clear(&vod_channels);
    vip_category_list_clear(&series_cats); vip_channel_list_clear(&series_channels);
