...
int main(void) {
    ...
    TEST_STATUS(vip_credentials_init(&creds, "https://example.com///", "alice", "secret", &error), VIP_OK,
                &error);
    ...
    TEST_STATUS(vip_credentials_init(&other, "https://example.com///", "bob", "secret", &error), VIP_OK,
                &error);
