...
int main(void) {
    ...
    vip_channel_t ch = {.provider_id = "p",
                        .id = "42",
                        .category_id = "7",
                        .name = "News",
                        .logo_url = "https://example/logo.png",
                        .stream_url = "https://example/live.ts",
                        .epg_channel_id = "news",
                        .position = 0};
    ...
    TEST_STATUS(vip_database_load_favorite_flags(db, "p", &loaded, favorite_flags, 1, &error), VIP_OK,
                &error);
    ...
    TEST_CHECK(!favorite_flags[0]);
    TEST_STATUS(
        vip_database_set_thumbnail(db, "p", "42", "/tmp/a.jpg", VIP_THUMB_CAPTURED_FRAME, 1, 2, &error),
        VIP_OK, &error);
    ...
    vip_profile_t profile = {.profile_id = "p",
                             .name = "Principal",
                             .type = VIP_PROFILE_XTREAM,
                             .server = "https://example",
                             .server_alt = "https://alt",
                             .username = "alice"};
    TEST_STATUS(vip_database_save_profile(db, &profile, &error), VIP_OK, &error);
    vip_profile_list_t profiles;
    vip_profile_list_init(&profiles);
    ...
    TEST_STATUS(vip_database_set_series_progress(db, "p", "series:7", "episode:2", 2, 10, &error), VIP_OK,
                &error);
    vip_series_progress_t series_progress = {0};
    TEST_STATUS(vip_database_get_series_progress(db, "p", "series:7", &series_progress, &error), VIP_OK,
                &error);
    ...
    vip_channel_list_t series_catalog;
    vip_channel_list_init(&series_catalog);
    vip_channel_t series_item = {
        .provider_id = "p", .id = "series:7", .name = "Série", .stream_url = "series://7", .position = 0};
    TEST_STATUS(vip_channel_list_push(&series_catalog, &series_item, &error), VIP_OK, &error);
    int watched[1] = {0}, total[1] = {0};
    TEST_STATUS(vip_database_load_series_progress(db, "p", &series_catalog, watched, total, 1, &error),
                VIP_OK, &error);
    TEST_CHECK(watched[0] == 2);
    TEST_CHECK(total[0] == 10);
    vip_channel_list_clear(&series_catalog);

    vip_media_metadata_t metadata = {.plot = "Sinopse persistida",
    ...
    vip_media_metadata_t loaded_metadata;
    vip_media_metadata_init(&loaded_metadata);
    int64_t metadata_updated = 0;
    bool metadata_found = false;
    TEST_STATUS(vip_database_get_media_metadata(db, "p", "42", &loaded_metadata, &metadata_updated,
                                                &metadata_found, &error),
                VIP_OK, &error);
