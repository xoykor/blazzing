...
int main(void) {
    ...
    TEST_CHECK(strcmp(channels.items[0].category_id, "pluto-live") == 0);
    TEST_CHECK(strcmp(channels.items[0].logo_url, "https://images.example/logo.png") == 0);
    TEST_CHECK(strstr(channels.items[0].stream_url, "/channel/abc123/master.m3u8") != NULL);
    TEST_CHECK(strstr(channels.items[0].stream_url, "jwt=token-test") != NULL);
    TEST_CHECK(channels.items[0].position == 42);
    TEST_CHECK(channels.items[1].logo_url == NULL);
