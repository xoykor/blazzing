...
static void draw_service_card(hub_window_t *h, int index, const char *monogram,
    ...
    if (h->renderer.active) {
        ...
        vip_ui_render_round_rect(&h->renderer, x, y, w, height, 22,
                                 selected ? 0x173B67u : 0x151E2Du, 0.99);
        vip_ui_render_round_stroke(&h->renderer, x, y, w, height, 22,
                                   selected ? 0x62A9FFu : 0x2B3950u, 1.0,
                                   selected ? 1.8 : 1.0);
        if (selected)
            vip_ui_render_round_stroke(&h->renderer, x + 3, y + 3, w - 6, height - 6, 19,
                                       0x8BC1FFu, 0.55, 1.0);

        vip_ui_render_round_rect(&h->renderer, x + 22, y + 22, 48, 48, 14,
                                 selected ? 0x62A9FFu : 0x0E1420u, 1.0);
        vip_ui_render_text(&h->renderer, x + 22, y + 35, 48, monogram, "Sans Bold 12",
                           selected ? 0x050811u : 0xF6F8FCu, 1.0, true);
        vip_ui_render_text(&h->renderer, x + 88, y + 29, w - 116, name, "Sans Bold 13",
                           0xF6F8FCu, 1.0, false);
        ...
        vip_ui_render_text(&h->renderer, x + 36, y + height - 48, w - 190, mode, "Sans SemiBold 8",
                           selected ? 0xDDE9F8u : 0x91A0B7u, 1.0, false);
        if (selected) {
            vip_ui_render_round_rect(&h->renderer, x + w - 128, y + height - 53, 92, 26, 9,
                                     0x62A9FFu, 1.0);
            vip_ui_render_text(&h->renderer, x + w - 128, y + height - 46, 92, "ABRIR  >",
                               "Sans Bold 8", 0x050811u, 1.0, true);
        }
        return;
    }
...
static void hub_draw(hub_window_t *h) {
    bool modern = vip_ui_renderer_begin(&h->renderer, h->dpy, h->win,
                                        DefaultVisual(h->dpy, h->screen),
                                        h->width, h->height);
    if (modern) {
        vip_ui_render_linear_gradient(&h->renderer, 0, 0, h->width, h->height,
                                      0x050811u, 0x090F1Au);
        ...
        vip_ui_render_text(&h->renderer, 116, 68, h->width - 168,
                           "IPTV e Pluto TV, sem atalhos para serviços externos",
                           "Sans 9", 0x91A0B7u, 1.0, false);

        vip_ui_render_text(&h->renderer, 54, 151, h->width - 108,
                           "Escolha uma fonte", "Sans Bold 14", 0xF6F8FCu, 1.0, false);
        vip_ui_render_text(&h->renderer, 54, 179, h->width - 108,
        ...
        hub_text(h, 54, 188, "O Blazzing mantem apenas reproducao que funciona de forma nativa dentro do aplicativo.", h->muted);
    }
    ...
    if (h->renderer.active) {
        ...
        vip_ui_render_text(&h->renderer, 54, footer_y + 27, h->width - 108,
                           "Setas: navegar    Enter: abrir    Esc: sair",
...
static int card_at(const hub_window_t *h, int px, int py) {
    for (int i = 0; i < 2; ++i) {
        ...
        if (point_in(px, py, x, y, w, height)) return i;
    }
    return -1;
}
...
static hub_action_t hub_window_run(void) {
    ...
    while (!done) {
        ...
        switch (event.type) {
            case Expose:
                if (event.xexpose.count == 0) hub_draw(&h);
                break;
    ...
    if (h.renderer.active) vip_ui_renderer_end(&h.renderer);
