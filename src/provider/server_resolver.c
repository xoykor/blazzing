...
/*
 * Fallback Xtream endpoint discovery and candidate verification.
 *
...
static const char *const resolver_apis[] = {
    "https://api.s-api.org",     "https://api2.s-api.org", "https://api.asplayer.xyz",
    "https://api2.asplayer.xyz", "https://api.spk-web.cc", "https://api2.spk-web.cc",
};

static const char resolver_code[] = "11";
static const char resolver_client[] = "firestream-tv";
static const char resolver_public_key[] = "QSxII7iGMtxErZEn35GRlstMCVXLn50IAmu1b1WOCDIpRrJRTwjrTmgXjREHFcMA";
...
static const char chrome_ua[] = "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) "
...
static size_t resolver_write(void *ptr, size_t size, size_t nmemb, void *userdata) {
    ...
    if (need > buf->cap) {
        ...
        while (cap < need && cap <= RESOLVER_MAX_RESPONSE / 2u)
            cap *= 2u;
        if (cap < need)
            cap = need;
...
static char *normalize_base(const char *input) {
    ...
    while (len && isspace((unsigned char)input[len - 1u]))
        --len;
    if (!len)
        return NULL;
...
static vip_status_t base_list_add(base_list_t *list, const char *value, vip_error_t *error) {
    ...
    if (list->len == list->cap) {
        ...
        if (!grown) {
            free(normalized);
            vip_error_set(error, VIP_ERR_NOMEM, "sem memória para servidores resolvidos");
            return VIP_ERR_NOMEM;
        }
        list->items = grown;
        list->cap = cap;
...
static vip_status_t collect_value(base_list_t *list, json_object *value, vip_error_t *error) {
    ...
    if (type == json_type_string)
        return base_list_add(list, json_object_get_string(value), error);
...
vip_status_t vip_streamfire_collect_bases(const char *json, char ***bases_out, size_t *count_out,
    ...
    *count_out = 0u;
    ...
    if (!root) {
        vip_error_set(error, VIP_ERR_MALFORMED, "JSON do resolvedor inválido");
        return VIP_ERR_MALFORMED;
    }
    base_list_t list = {0};
    ...
    if (st != VIP_OK) {
        vip_streamfire_free_bases(list.items, list.len);
        return st;
    }
    *bases_out = list.items;
    *count_out = list.len;
    vip_error_clear(error);
    return VIP_OK;
}
...
static char *identity_file_path(void) {
    ...
    char *base = NULL;
    if (config && config[0])
        base = vip_strdup(config);
    else if (home && home[0]) {
        size_t n = strlen(home) + 9u;
        base = malloc(n);
        if (base)
            snprintf(base, n, "%s/.config", home);
    }
    if (!base)
        return NULL;
    ...
    char *path = malloc(pn);
    if (path)
        snprintf(path, pn, "%s/identity.txt", dir);
    free(dir);
    return path;
}
...
static vip_status_t load_identity(char out[192], vip_error_t *error) {
    ...
    if (!path) {
        vip_error_set(error, VIP_ERR_IO, "não foi possível preparar identidade do resolvedor");
        return VIP_ERR_IO;
    }
    FILE *fp = fopen(path, "rb");
    if (fp) {
        size_t n = fread(out, 1u, 191u, fp);
        fclose(fp);
        while (n && (out[n - 1u] == '\n' || out[n - 1u] == '\r' || isspace((unsigned char)out[n - 1u])))
            --n;
        out[n] = '\0';
        bool valid = n > 0u;
        for (size_t i = 0; valid && i < n; ++i)
            valid = valid_identity_char((unsigned char)out[i]);
        if (valid) {
            free(path);
            return VIP_OK;
        }
    ...
    if (RAND_bytes(r, sizeof(r)) != 1) {
        free(path);
        vip_error_set(error, VIP_ERR_IO, "falha ao gerar identidade do resolvedor");
        return VIP_ERR_IO;
    }
    r[6] = (unsigned char)((r[6] & 0x0fu) | 0x40u);
    r[8] = (unsigned char)((r[8] & 0x3fu) | 0x80u);
    snprintf(out, 192u, "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x", r[0], r[1],
...
static struct curl_slist *resolver_headers(int profile) {
    ...
    char client[128];
    snprintf(client, sizeof(client), "X-Client: %s", resolver_client);
    headers = curl_slist_append(headers, client);
    char key[256];
    snprintf(key, sizeof(key), "X-Public-Key: %s", resolver_public_key);
    headers = curl_slist_append(headers, key);
    if (profile >= 1) {
        char origin[160];
        snprintf(origin, sizeof(origin), "Origin: %s", resolver_origin);
        char referer[200];
        snprintf(referer, sizeof(referer), "Referer: %s", resolver_referer);
...
static bool response_payload(const char *body, char **payload_out) {
    *payload_out = NULL;
    ...
    if (json_object_object_get_ex(root, "result", &result) && result &&
        json_object_get_type(result) == json_type_boolean && !json_object_get_boolean(result)) {
        json_object_put(root);
        return false;
    }
    json_object *response = NULL;
    bool ok = json_object_object_get_ex(root, "response", &response) && response &&
              json_object_get_type(response) == json_type_string;
    if (ok)
        *payload_out = vip_strdup(json_object_get_string(response));
    json_object_put(root);
...
static bool verify_xtream_base(const char *base, const char *username, const char *password) {
    ...
    size_t n = strlen(base) + strlen(eu) + strlen(ep) + 64u;
    char *url = malloc(n);
    if (!url) {
        curl_free(eu);
        curl_free(ep);
    ...
    curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
    curl_easy_setopt(curl, CURLOPT_USERAGENT, chrome_ua);
    curl_easy_setopt(curl, CURLOPT_NOSIGNAL, 1L);
    CURLcode rc = curl_easy_perform(curl);
    long http = 0;
    ...
    free(url);
    bool ok = false;
    if (rc == CURLE_OK && http >= 200 && http < 300 && buf.data) {
        vip_error_t ignored = {0};
        ok = vip_xtream_parse_auth_json(buf.data, &ignored) == VIP_OK;
    }
    free(buf.data);
    return ok;
}
