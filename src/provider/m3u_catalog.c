...
vip_m3u_content_kind_t vip_m3u_classify_group(const char *group_name) {
    ...
    if (has_word(norm, "series") || has_word(norm, "serie") ||
        has_word(norm, "seriados") || has_word(norm, "seriado") ||
        has_word(norm, "anime") || has_word(norm, "animes") ||
        has_word(norm, "novela") || has_word(norm, "novelas") ||
        has_word(norm, "dorama") || has_word(norm, "doramas"))
        return VIP_M3U_CONTENT_SERIES;
    if (has_word(norm, "filmes") || has_word(norm, "filme") ||
        has_word(norm, "movies") || has_word(norm, "movie") ||
        has_word(norm, "vod") || has_word(norm, "cinema"))
...
static bool parse_uint_at(const char *s, size_t len, size_t *pos, int *value) {
    if (!s || !pos || *pos >= len || !isdigit((unsigned char)s[*pos])) return false;
    int v = 0;
    size_t i = *pos;
    int digits = 0;
...
static bool episode_marker_at(const char *name, size_t len, size_t pos,
                              size_t *end_out, int *season_out, int *episode_out) {
    if (!name || pos >= len) return false;
    unsigned char c = (unsigned char)name[pos];
    if (c == 's' || c == 'S' || c == 't' || c == 'T') {
        size_t p = pos + 1u;
        int season = 0, episode = 0;
        if (!parse_uint_at(name, len, &p, &season)) return false;
        while (p < len && (name[p] == ' ' || name[p] == '.' || name[p] == '-' || name[p] == '_')) ++p;
        if (p >= len || (name[p] != 'e' && name[p] != 'E')) return false;
        ++p;
        if (!parse_uint_at(name, len, &p, &episode)) return false;
        if (end_out) *end_out = p;
        if (season_out) *season_out = season;
        if (episode_out) *episode_out = episode;
        return true;
    }
    if (isdigit(c)) {
        size_t p = pos;
        int season = 0, episode = 0;
        if (!parse_uint_at(name, len, &p, &season)) return false;
        if (p >= len || (name[p] != 'x' && name[p] != 'X')) return false;
        ++p;
        if (!parse_uint_at(name, len, &p, &episode)) return false;
        if (end_out) *end_out = p;
        if (season_out) *season_out = season;
...
bool vip_m3u_parse_episode_label(const char *name,
                                 ...
                                 int *season_out,
                                 int *episode_out) {
    if (series_out && series_cap > 0u) series_out[0] = '\0';
    if (!name || !name[0] || !series_out || series_cap == 0u) return false;
    ...
    if (marker == len) return false;
    size_t title_end = marker;
    while (title_end > 0u) {
        unsigned char c = (unsigned char)name[title_end - 1u];
        if (isspace(c) || c == '-' || c == '_' || c == '|' || c == '.' || c == ':') --title_end;
        else break;
    }
    if (title_end == 0u) return false;
...
static const vip_category_t *find_category(const vip_category_list_t *cats, const char *id) {
    ...
    for (size_t i = 0u; i < cats->len; ++i)
        if (cats->items[i].id && strcmp(cats->items[i].id, id) == 0) return &cats->items[i];
    return NULL;
}

static bool category_is_used(const vip_channel_list_t *channels, const char *category_id) {
    if (!channels || !category_id) return false;
    for (size_t i = 0u; i < channels->len; ++i)
        if (channels->items[i].category_id && strcmp(channels->items[i].category_id, category_id) == 0) return true;
    return false;
}

static vip_status_t push_channel_for_kind(vip_m3u_content_kind_t kind,
                                          ...
                                          vip_error_t *error) {
    if (kind == VIP_M3U_CONTENT_VOD) return vip_channel_list_push(vod_channels, channel, error);
    if (kind == VIP_M3U_CONTENT_SERIES) return vip_channel_list_push(series_channels, channel, error);
...
vip_status_t vip_m3u_split_catalog(const vip_category_list_t *source_categories,
                                   ...
                                   vip_channel_list_t *series_channels,
                                   vip_error_t *error) {
    if (!source_categories || !source_channels || !live_categories || !live_channels ||
        !vod_categories || !vod_channels || !series_categories || !series_channels) {
...
fail:
    vip_category_list_clear(live_categories); vip_category_list_init(live_categories);
    vip_channel_list_clear(live_channels); vip_channel_list_init(live_channels);
    vip_category_list_clear(vod_categories); vip_category_list_init(vod_categories);
    vip_channel_list_clear(vod_channels); vip_channel_list_init(vod_channels);
    vip_category_list_clear(series_categories); vip_category_list_init(series_categories);
    vip_channel_list_clear(series_channels); vip_channel_list_init(series_channels);
    return st;
}
