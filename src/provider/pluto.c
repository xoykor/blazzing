...
/*
 * Pluto TV bootstrap, channel discovery and stream URL provider.
 *
...
#define PLUTO_CHANNELS_URL                                                                                   \
    "https://service-channels.clusters.pluto.tv/v2/guide/"                                                   \
    "channels?channelIds=&offset=0&limit=1000&sort=number%3Aasc"
...
static size_t write_response(void *ptr, size_t size, size_t nmemb, void *userdata) {
    ...
    if (need > buf->cap) {
        ...
        while (cap < need && cap <= PLUTO_MAX_RESPONSE / 2u)
            cap *= 2u;
        if (cap < need)
            cap = need;
...
static void random_client_id(char out[33]) {
    ...
    if (RAND_bytes(bytes, (int)sizeof(bytes)) != 1) {
        for (size_t i = 0; i < sizeof(bytes); ++i)
            bytes[i] = (unsigned char)(17u * i + 31u);
    }
    for (size_t i = 0; i < sizeof(bytes); ++i)
    ...
    *body_out = NULL;
    ...
    if (rc != CURLE_OK) {
        ...
        vip_error_set(error, VIP_ERR_NETWORK,
                      buf.overflow ? "resposta do Pluto excedeu 32 MiB" : "falha HTTP Pluto: %s",
...
vip_status_t vip_pluto_boot(vip_pluto_client_t *client, vip_error_t *error) {
    ...
    (void)snprintf(url, sizeof(url),
                   PLUTO_BOOT_URL "?appName=web&appVersion=8.0.0&deviceVersion=122.0.0"
    ...
    if (!stitcher)
        stitcher = vip_strdup(PLUTO_STITCHER_FALLBACK);
    if (!stitcher) {
        free(token);
        free(params);
...
static const char *channel_logo(json_object *channel) {
    ...
    if (json_object_object_get_ex(channel, "images", &images) && images &&
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            if (url[0] && (!strcmp(type, "colorLogoPNG") || !strcmp(type, "logo")))
                return url;
        }
    }
    ...
    if (json_object_object_get_ex(channel, "logo", &logo) && logo &&
        json_object_get_type(logo) == json_type_object)
        return jstr(logo, "path");
    return "";
}

/* Append channel. */
static vip_status_t append_channel(json_object *obj, size_t index, const char *session_token,
                                   const char *stitcher_base, vip_channel_list_t *out, vip_error_t *error) {
    ...
    (void)snprintf(stream, url_n, "%s/v2/stitch/hls/channel/%s/master.m3u8?jwt=%s&masterJWTPassthrough=true",
    ...
    vip_channel_t item = {.provider_id = (char *)PLUTO_PROVIDER_ID,
                          .id = (char *)id,
...
vip_status_t vip_pluto_live_catalog(vip_pluto_client_t *client, vip_category_list_t *categories_out,
    ...
    vip_category_t category = {.provider_id = (char *)PLUTO_PROVIDER_ID,
                               .id = (char *)"pluto-live",
                               .name = (char *)"Pluto TV",
                               .position = 0};
    st = vip_category_list_push(categories_out, &category, error);
    if (st == VIP_OK)
        st = vip_pluto_parse_channels_json(
            body, client->session_token,
            client->stitcher_base ? client->stitcher_base : PLUTO_STITCHER_FALLBACK, channels_out, error);
    free(body);
    return st;
}
