...
static uint64_t key_hash(const char *a, const char *b) {
    ...
    h ^= 0xff;
    h *= UINT64_C(1099511628211);
    for (const unsigned char *p = (const unsigned char *)b; p && *p; ++p) {
        h ^= *p;
        h *= UINT64_C(1099511628211);
    }
    return h;
...
static vip_status_t heap_push(vip_thumbnail_scheduler_t *s, queue_item_t item, vip_error_t *error) {
    ...
    while (i > 0) {
        size_t parent = (i - 1) / 2;
        if (!queue_higher(&s->heap[i], &s->heap[parent]))
            break;
        queue_item_t tmp = s->heap[i];
...
static queue_item_t heap_pop(vip_thumbnail_scheduler_t *s) {
    ...
    for (;;) {
        ...
        if (left < s->heap_len && queue_higher(&s->heap[left], &s->heap[best]))
            best = left;
        if (right < s->heap_len && queue_higher(&s->heap[right], &s->heap[best]))
            best = right;
        if (best == i)
            break;
        queue_item_t tmp = s->heap[i];
...
static void *worker_main(void *userdata) {
    ...
    while (next_request(s, &request)) {
        ...
        char *path = NULL;
        vip_status_t st =
            s->capture ? s->capture(&request, &path, &error, s->capture_userdata) : VIP_ERR_INVALID_ARGUMENT;
        if (s->ready)
            s->ready(&request, st, path, &error, s->ready_userdata);
        free(path);
        pthread_mutex_lock(&s->mutex);
...
vip_status_t vip_thumbnail_scheduler_create(vip_thumbnail_scheduler_t **out, size_t worker_count,
    ...
    *out = NULL;
    ...
    s->capture = capture;
    s->capture_userdata = capture_userdata;
    s->ready = ready;
    s->ready_userdata = ready_userdata;
    ...
    for (; created < s->worker_count; ++created) {
        if (pthread_create(&s->workers[created], NULL, worker_main, s) != 0)
            break;
    }
...
void vip_thumbnail_scheduler_destroy(vip_thumbnail_scheduler_t *s) {
    ...
    for (size_t i = 0; i < s->heap_len; ++i)
        request_clear(&s->heap[i].request);
    free(s->heap);
    free(s->workers);
...
void vip_thumbnail_scheduler_cancel_pending(vip_thumbnail_scheduler_t *s) {
    ...
    for (size_t i = 0; i < s->heap_len; ++i) {
        ...
        if (e && !e->generating)
            map_remove(s, r->provider_id, r->channel_id);
        request_clear(r);
    }
    s->heap_len = 0u;
    pthread_mutex_unlock(&s->mutex);
...
char *vip_thumbnail_cache_path(const char *cache_dir, const char *provider_id, const char *channel_id,
    ...
    char *path = malloc(n);
    if (!path) {
        vip_error_set(error, VIP_ERR_NOMEM, "sem memória para caminho de thumbnail");
        return NULL;
    }
...
static vip_status_t read_local_logo(const char *source, image_download_t *buf, vip_error_t *error) {
    ...
    FILE *fp = fopen(path, "rb");
    if (!fp) {
        vip_error_set(error, VIP_ERR_IO, "não foi possível abrir capa local");
        return VIP_ERR_IO;
    }
...
/* Decode jpeg memory. */
static vip_status_t decode_jpeg_memory(const uint8_t *data, size_t len, decoded_image_t *out,
                                       vip_error_t *error) {
...
/* Decode png memory. */
static vip_status_t decode_png_memory(const uint8_t *data, size_t len, decoded_image_t *out,
                                      vip_error_t *error) {
    png_image image;
    memset(&image, 0, sizeof(image));
    ...
    out->stride = (size_t)image.width * 3u;
    png_image_free(&image);
    vip_error_clear(error);
    return VIP_OK;
}
...
/* Decode webp memory. */
static vip_status_t decode_webp_memory(const uint8_t *data, size_t len, decoded_image_t *out,
                                       vip_error_t *error) {
    int width = 0, height = 0;
    if (!WebPGetInfo(data, len, &width, &height) || width <= 0 || height <= 0 || width > 8192 ||
        height > 8192)
        return VIP_ERR_INVALID_FRAME;
    ...
    if (!rgb)
        return VIP_ERR_NOMEM;
    if (!WebPDecodeRGBInto(data, len, rgb, bytes, (int)stride)) {
        free(rgb);
        return VIP_ERR_INVALID_FRAME;
    }
    out->data = rgb;
...
/* Decode image memory. */
static vip_status_t decode_image_memory(const uint8_t *data, size_t len, decoded_image_t *out,
                                        vip_error_t *error) {
    if (!data || len < 12u || !out)
        return VIP_ERR_INVALID_ARGUMENT;
    memset(out, 0, sizeof(*out));
    if (len >= 8u && png_sig_cmp((png_bytep)data, 0, 8) == 0)
        return decode_png_memory(data, len, out, error);
    if (data[0] == 0xffu && data[1] == 0xd8u)
        return decode_jpeg_memory(data, len, out, error);
...
static vip_status_t save_rgb_jpeg_exact(const uint8_t *rgb, size_t width, size_t height, size_t stride,
    ...
    if (close_rc != 0 || rename_rc != 0) {
        ...
        (void)remove(tmp_path);
        free(tmp_path);
        vip_error_set(error, VIP_ERR_IO, "falha ao publicar capa no cache: %s", strerror(saved_errno));
        return VIP_ERR_IO;
    }
    free(tmp_path);
    vip_error_clear(error);
...
static vip_status_t capture_logo_direct(const char *logo_url, const char *path, int quality,
    ...
    vip_status_t st = (strncmp(logo_url, "http://", 7u) == 0 || strncmp(logo_url, "https://", 8u) == 0)
                          ? download_logo(logo_url, &raw, error)
                          : read_local_logo(logo_url, &raw, error);
    if (st != VIP_OK) {
        free(raw.data);
        return st;
    }
    ...
    free(raw.data);
    if (st == VIP_OK)
        st = save_logo_preserving_aspect(&image, path, quality, error);
...
static bool failure_backoff_active(const char *path) {
    ...
    if (stat(marker, &st) == 0) {
        ...
        active = now != (time_t)-1 && st.st_mtime <= now && now - st.st_mtime < THUMB_FAILURE_BACKOFF_SECONDS;
        if (!active)
            (void)remove(marker);
    }
    free(marker);
