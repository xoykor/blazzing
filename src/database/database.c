...
static vip_status_t migrate(vip_database_t *db, vip_error_t *error) {
    const char *sql =
        ...
        " provider_id TEXT NOT NULL, channel_id TEXT NOT NULL, created_at INTEGER NOT NULL "
        "DEFAULT(unixepoch()),"
        " PRIMARY KEY(provider_id, channel_id));"
        "CREATE TABLE IF NOT EXISTS history("
        " provider_id TEXT NOT NULL, channel_id TEXT NOT NULL, watched_at INTEGER NOT NULL "
        "DEFAULT(unixepoch()),"
        " PRIMARY KEY(provider_id, channel_id));"
        ...
        "CREATE INDEX IF NOT EXISTS watch_progress_updated_idx ON watch_progress(provider_id, updated_at "
        "DESC);"
        ...
        " genre TEXT, release_date TEXT, rating TEXT, duration TEXT, cast TEXT, director TEXT, "
...
vip_status_t vip_database_set_favorite(vip_database_t *db, const char *provider_id, const char *channel_id,
    ...
    sqlite3_stmt *stmt = NULL;
    const char *sql =
        favorite ? "INSERT INTO favorites(provider_id,channel_id) VALUES(?1,?2) ON CONFLICT DO NOTHING"
    ...
    pthread_mutex_unlock(&db->mutex);
    if (rc != SQLITE_DONE)
        return db_error(db, error, "falha ao atualizar favorito");
    return VIP_OK;
}
...
vip_status_t vip_database_set_thumbnail(vip_database_t *db, const char *provider_id, const char *channel_id,
    ...
    if (!db || !provider_id || !channel_id || !path)
        return VIP_ERR_INVALID_ARGUMENT;
    pthread_mutex_lock(&db->mutex);
    sqlite3_stmt *stmt = NULL;
    const char *sql =
    ...
    pthread_mutex_unlock(&db->mutex);
    if (rc != SQLITE_DONE)
        return db_error(db, error, "falha ao salvar thumbnail");
    return VIP_OK;
}
...
vip_status_t vip_database_load_favorite_flags(vip_database_t *db, const char *provider_id,
    ...
    pthread_mutex_lock(&db->mutex);
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(db->conn, "SELECT channel_id FROM favorites WHERE provider_id=?1", -1, &stmt,
                           NULL) != SQLITE_OK) {
        pthread_mutex_unlock(&db->mutex);
...
char *vip_database_thumbnail_path(vip_database_t *db, const char *provider_id, const char *channel_id,
    ...
    if (sqlite3_prepare_v2(db->conn,
                           "SELECT path FROM thumbnail_metadata WHERE provider_id=?1 AND channel_id=?2", -1,
                           &stmt, NULL) != SQLITE_OK) {
        pthread_mutex_unlock(&db->mutex);
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (text)
            path = vip_strdup((const char *)text);
    }
    sqlite3_finalize(stmt);
    pthread_mutex_unlock(&db->mutex);
    return path;
...
vip_status_t vip_database_save_profile(vip_database_t *db, const vip_profile_t *profile, vip_error_t *error) {
    ...
    pthread_mutex_lock(&db->mutex);
    sqlite3_stmt *stmt = NULL;
    const char *sql = "INSERT INTO profiles(profile_id,name,type,server,server_alt,username,last_used)"
                      " VALUES(?1,?2,?3,?4,?5,?6,unixepoch())"
                      " ON CONFLICT(profile_id) DO UPDATE SET name=excluded.name,type=excluded.type,"
                      " server=excluded.server,server_alt=excluded.server_alt,username=excluded.username,"
                      "last_used=unixepoch()";
    if (sqlite3_prepare_v2(db->conn, sql, -1, &stmt, NULL) != SQLITE_OK) {
        pthread_mutex_unlock(&db->mutex);
    ...
    pthread_mutex_unlock(&db->mutex);
    if (rc != SQLITE_DONE)
        return db_error(db, error, "falha ao salvar perfil");
    vip_error_clear(error);
    return VIP_OK;
}
...
vip_status_t vip_database_list_profiles(vip_database_t *db, vip_profile_list_t *out, vip_error_t *error) {
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(db->conn,
                           "SELECT profile_id,name,type,server,server_alt,username,last_used FROM profiles "
...
vip_status_t vip_database_touch_profile(vip_database_t *db, const char *profile_id, vip_error_t *error) {
    ...
    pthread_mutex_lock(&db->mutex);
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(db->conn, "UPDATE profiles SET last_used=unixepoch() WHERE profile_id=?1", -1,
                           &stmt, NULL) != SQLITE_OK) {
        pthread_mutex_unlock(&db->mutex);
...
vip_status_t vip_database_set_progress(vip_database_t *db, const char *provider_id, const char *channel_id,
    ...
    const char *sql =
        "INSERT INTO "
        "watch_progress(provider_id,channel_id,position_seconds,duration_seconds,completed,updated_at)"
    ...
    pthread_mutex_unlock(&db->mutex);
    if (rc != SQLITE_DONE)
        return db_error(db, error, "falha ao salvar progresso");
    vip_error_clear(error);
    return VIP_OK;
}
...
vip_status_t vip_database_get_progress(vip_database_t *db, const char *provider_id, const char *channel_id,
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(db->conn,
                           "SELECT position_seconds,duration_seconds,completed,updated_at FROM "
                           "watch_progress WHERE provider_id=?1 AND channel_id=?2",
                           -1, &stmt, NULL) != SQLITE_OK) {
        pthread_mutex_unlock(&db->mutex);
...
vip_status_t vip_database_load_progress(vip_database_t *db, const char *provider_id,
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(db->conn,
                           "SELECT channel_id,position_seconds,duration_seconds,completed,updated_at FROM "
                           "watch_progress WHERE provider_id=?1",
                           -1, &stmt, NULL) != SQLITE_OK) {
        pthread_mutex_unlock(&db->mutex);
...
vip_status_t vip_database_set_series_progress(vip_database_t *db, const char *provider_id,
    ...
    const char *sql =
        "INSERT INTO "
        "series_progress(provider_id,series_id,last_episode_id,watched_count,total_count,updated_at)"
        " VALUES(?1,?2,?3,?4,?5,unixepoch())"
    ...
    pthread_mutex_unlock(&db->mutex);
    if (rc != SQLITE_DONE)
        return db_error(db, error, "falha ao salvar progresso da série");
    vip_error_clear(error);
    return VIP_OK;
}
...
vip_status_t vip_database_load_series_progress(vip_database_t *db, const char *provider_id,
    ...
    if (sqlite3_prepare_v2(
            db->conn, "SELECT series_id,watched_count,total_count FROM series_progress WHERE provider_id=?1",
            -1, &stmt, NULL) != SQLITE_OK) {
        pthread_mutex_unlock(&db->mutex);
...
vip_status_t vip_database_get_series_progress(vip_database_t *db, const char *provider_id,
                                              ...
                                              vip_error_t *error) {
    if (!db || !provider_id || !series_id || !out)
        return VIP_ERR_INVALID_ARGUMENT;
    memset(out, 0, sizeof(*out));
    pthread_mutex_lock(&db->mutex);
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(db->conn,
                           "SELECT last_episode_id,watched_count,total_count,updated_at FROM series_progress "
                           "WHERE provider_id=?1 AND series_id=?2",
                           -1, &stmt, NULL) != SQLITE_OK) {
        pthread_mutex_unlock(&db->mutex);
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (last)
            out->last_episode_id = vip_strdup((const char *)last);
...
vip_status_t vip_database_get_media_metadata(vip_database_t *db, const char *provider_id,
    ...
    const char *sql = "SELECT "
                      "plot,cover_url,backdrop_url,genre,release_date,rating,duration,[cast],director,"
                      "youtube_trailer,updated_at"
    ...
    if (rc == SQLITE_ROW) {
        ...
        if (updated_at_out)
            *updated_at_out = sqlite3_column_int64(stmt, 10);
        if (found_out)
            *found_out = true;
    }
    sqlite3_finalize(stmt);
