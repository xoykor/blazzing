...
/*
 * Cairo/Pango rendering primitives shared by the native X11 interfaces.
 *
...
/* Set layout font. */
static PangoFontDescription *set_layout_font(vip_ui_renderer_t *renderer, const char *text,
                                             const char *font) {
...
void vip_ui_renderer_end(vip_ui_renderer_t *renderer) {
    if (!renderer)
        return;
    if (renderer->layout)
        g_object_unref(layout_of(renderer));
    if (renderer->context)
        cairo_destroy(context_of(renderer));
    if (renderer->surface)
        cairo_surface_destroy(surface_of(renderer));
    memset(renderer, 0, sizeof(*renderer));
}
...
void vip_ui_render_round_stroke(vip_ui_renderer_t *renderer, int x, int y, int w, int h, int radius,
    ...
    sync_external_draw(renderer);
    cairo_t *cr = context_of(renderer);
    rounded_path(cr, x + line_width * 0.5, y + line_width * 0.5, w - line_width, h - line_width, radius);
    set_rgb(cr, rgb, alpha);
